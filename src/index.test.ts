...
test('should return "bg-foreground" from getClassName("root") function', () => {
  const props: Props = {
...
test('should return "text-red-500" from getClassName("icon") function because he is the default', () => {
  const props: Props = {
    className: 'text-red-500',
  };

  const getClassName = buildClassNames(props.className, 'icon');

  expect(getClassName('icon')).toBeUndefined();
});
